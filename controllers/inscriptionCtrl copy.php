...
if ($mdp2 == null || $mdp2 != $mdp1) {
    $messageKOMdp2 .= "<br/>Mot de passe incorrect ou différent";
    $nKO++;
}
...
if ($nKO === 0) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && $nKO === 0) {
        ...
        if ($statement->rowCount() > 0) {
            ...
            $salarie = new Salariees(null, $pseudo, $email1, $mdp1, $mobile, $poste, $cp, $ville);

            // Insertion des données dans la base de données
            // $salarieesID = $SalarieesDAO->insert($data);
            $salarieesID = $SalarieesDAO->insert($salarie);
